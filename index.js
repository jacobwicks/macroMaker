...
app.get("/macro/:input/:url*", async (req, res) => {
  const { params } = req;
...
//defining the response to an http 'get' request
app.get("/", (req, res) =>
  res.send("You have reached the Image Macro Generator")
);

//start the web server listening
app.listen(port, () => {
  console.log(`Image Macro Generator listening at on port ${port}`);
});
